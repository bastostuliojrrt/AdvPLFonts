  ...
  For nLoop := 1 To Len(aTitulos)
	 ...
	 // Se estiver marcado, imprime
      If aTitulos[nLoop,01]
        ...
        // -- Verificar se o beneficiário é o mesmo da emissão
        // ---------------------------------------------------
         If !Empty(SEE->EE_XBENEF)
            ...
            If (aCodBene[01])->(dbSeek(FWxFilial(aCodBene[01]) +;
               ...
               aEmpresa := {AllTrim(&(cAliaBen + "_NOME")),;                      // 01 = Nome da Empresa
                            AllTrim(&(cAliaBen + "_END")),;                       // 02 = Endereço
...
/*  If aBanco[1] == "104"
  ...
  oPrint:Say(nRow2 + 0910, 1810, IIf(aBanco[1] == "104","Nosso Número","Carteira / Nosso Número"), oFont08)
...
Static Function Modulo10(cData)
  ...
  EndDo
   ...
   if cQualBco == "748"
      ...
      if mod(D,10) != 0            // Se o somatório não for múltiplo de 10
         M := D
         while mod(M,10) != 0
            M++
         EndDo
         D := M - D
      else                    // Se o somatório for múltiplo de 10
         D := D - D
      EndIf
   else
       D := 10 - (Mod(D,10))
...
// Modulo da divisao da soma dos digitos por 11. O resultado é subtraido de 11.
   D := 11 - (mod(D,11))
